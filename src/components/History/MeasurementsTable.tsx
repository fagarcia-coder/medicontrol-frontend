      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4 items-end">
        ...
        <div className="flex gap-2">
          <div>
            ...
            <input type="number" value={minValue} onChange={(e) => setMinValue(e.target.value)} className="w-24 border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-sky-300" />
          </div>
          <div>
            <label className="block text-sm text-gray-600">Valor max</label>
            <input type="number" value={maxValue} onChange={(e) => setMaxValue(e.target.value)} className="w-24 border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-sky-300" />
          </div>
        </div>
        <div className="flex gap-3 justify-end items-center">
          <div>
            <label className="block text-sm text-gray-600">Desde</label>
            <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-sky-300" />
          </div>
          <div>
            <label className="block text-sm text-gray-600">Hasta</label>
            <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-sky-300" />
          </div>
        </div>
            ...
            {pageItems.map((m) => {
              ...
              return (
                <tr key={m.id} className="border-t last:border-b hover:bg-gray-50">
                  <td className="px-4 py-3 align-top w-1/3 text-sm text-gray-700">{new Date(m.created_at).toLocaleString()}</td>
                  <td className="px-4 py-3 align-top w-1/6 text-right">
                  ...
                  <td className="px-4 py-3 align-top w-1/2 text-sm text-gray-700">{m.note || '-'}</td>
